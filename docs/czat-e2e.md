...
| 6 — wyszukiwanie lokalne + TOFU | **zrobione (webappka)**: `e2eKeys.ts` (store `peerKeys` w IndexedDB v2, `observePeerKey` → `'new'\|'same'\|'changed'`, `fingerprintOf` = SHA-256 w 4 grupach), `chatService.watchPeerKey` (nasłuch na `usersPublic.{uid}.chatKey.pub` przez `onSnapshot`), `ChatPage.tsx` (pole szukania filtrujące odszyfrowane wiadomości + baner `chat.e2eKeyChanged` przy `status==='changed'`). ⚠️ **Android: NIE zrobione** — brakuje lokalnego szukania w wątku i ostrzeżenia TOFU |
| 7 — teksty w UI | **zrobione (webappka)**: `E2ePanel.tsx` (zwijany blok pomocy `chat.e2eLearnMore` + `chat.e2eHelp*`), 11 kluczy i18n we wszystkich 6 językach (`chat.search*`, `chat.e2eKeyChanged*`, `chat.e2eLearnMore`, `chat.e2eHelp*`), sekcja „Szyfrowanie end-to-end w czacie" w polityce prywatności (`build_privacy.py`, wszystkie 6 języków, data 2026-09-12). Wdrożone na produkcję (`deploy:web`). ⚠️ **Android: NIE zrobione** — brakuje pomocy w czacie, tekstów TOFU i sekcji E2E w polityce aplikacji |
